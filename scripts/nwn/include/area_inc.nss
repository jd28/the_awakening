...
const string sPINS  = "NW_TOTAL_MAP_PINS"; // Int
const string sENTRY = "NW_MAP_PIN_NTRY_"; // String - Pin name.
const string sXPOS  = "NW_MAP_PIN_XPOS_"; // Float
const string sYPOS  = "NW_MAP_PIN_YPOS_"; // Float
const string sZPOS  = "NW_MAP_PIN_ZPOS_"; // Float -- Always 0.0
const string sAREA  = "NW_MAP_PIN_AREA_"; // Area Object

void AreaUpdateMapPins(object oPC, object oArea);
void AreaAddMapPin(object oPC, object oArea, string sEntry = "");

// pc_name | area_res | mp_x | mp_y | mp_entry
void AreaAddMapPin(object oPC, object oArea, string sEntry = ""){
    string sName = SQLEncodeSpecialChars(GetPCPlayerName(oPC));
    string sAreaRes = SQLEncodeSpecialChars(GetResRef(oArea));
    vector v = GetPositionFromLocation(GetLocation(oPC));
    int x = FloatToInt(v.x) , y = FloatToInt(v.y);
    string sSQL;

    if(SQLFetch() != SQL_ERROR){
        // row exists
        sSQL = "UPDATE pc_map_pin " +
            "SET mp_x='" + IntToString(x) +
            "SET mp_y='" + IntToString(y) +
            "SET mp_entry='" + sEntry +
            " WHERE pc_name='" + sName + "' AND area_res='" + sAreaRes + "'";
        SQLExecDirect(sSQL);
    }
    else{
        // row doesn't exist
        sSQL = "INSERT INTO pc_map_pin (pc_name,area_res,mp_x,mp_y,mp_entry) VALUES" +
            "('" + sName + "','" + sAreaRes + "','" + IntToString(x) + "'," + IntToString(y) + sEntry + ")";
        SQLExecDirect(sSQL);
    }
}

void AreaClearAreaMappins(object oPC, object oArea);
void AreaClearAreaMappins(object oPC, object oArea){
    string sName = SQLEncodeSpecialChars(GetPCPlayerName(oPC));
    string sAreaRes = SQLEncodeSpecialChars(GetResRef(oArea));

    string sSQL = "DELETE FROM pc_map_pin WHERE pc_name='" + sName + "' AND area_res='" + sAreaRes + "'";
    SQLExecDirect(sSQL);
}

void AreaClearAllMappins(object oPC);
void AreaClearAllMappins(object oPC){
    string sName = SQLEncodeSpecialChars(GetPCPlayerName(oPC));
    string sSQL = "DELETE FROM pc_map_pin WHERE pc_name='" + sName + "'";

    SQLExecDirect(sSQL);
}
void AreaUpdateMapPins(object oPC, object oArea){
    //Delete old pins
    int i, nCount = GetLocalInt(oPC, sPINS);

    for(i = 1; i <= nCount; i++){
        DeleteLocalInt(oPC, sPINS);
        DeleteLocalString(oPC,sENTRY+IntToString(i));
        DeleteLocalFloat(oPC,sXPOS+IntToString(i));
        DeleteLocalFloat(oPC,sYPOS+IntToString(i));
        DeleteLocalObject(oPC,sAREA+IntToString(i));
    }

    // Grab for this area
    // | inc | pc_name | area_res | mp_x | mp_y | mp_entry

    string sName = SQLEncodeSpecialChars(GetPCPlayerName(oPC));
    string sAreaRes = SQLEncodeSpecialChars(GetResRef(oArea));

    string sSQL = "SELECT mp_x, mp_y, mp_entry FROM pc_map_pin WHERE pc_name='" + sName + "' AND area_res='" + sAreaRes + "'";
    SQLExecDirect(sSQL);

    nCount = 0;
    while (SQLFetch() != SQL_ERROR){
        SetLocalFloat(oPC,sXPOS+IntToString(nCount), StringToFloat(SQLDecodeSpecialChars(SQLGetData(1))));
        SendMessageToPC(oPC, SQLDecodeSpecialChars(SQLGetData(1)));
        SetLocalFloat(oPC,sYPOS+IntToString(nCount), StringToFloat(SQLDecodeSpecialChars(SQLGetData(2))));
        SendMessageToPC(oPC, SQLDecodeSpecialChars(SQLGetData(2)));
        SetLocalString(oPC,sENTRY+IntToString(nCount), SQLDecodeSpecialChars(SQLGetData(3)));
        SendMessageToPC(oPC, SQLDecodeSpecialChars(SQLGetData(3)));
        SetLocalObject(oPC,sAREA+IntToString(nCount), oArea);

        nCount++;
    }
    SetLocalInt(oPC, sPINS, nCount);
}
