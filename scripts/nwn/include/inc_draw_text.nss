...
// fRotate = angle of rotation
void TextMessage(string sMessage, object oData, float fSpeed=2.0f, float fLifetime=0.0f, float fFontHeight=0.5f, float fFontWidth=0.25f, int nVFX=VFX_BEAM_FIRE_W_SILENT, float fRotate=0.0f);

// Assigns oData to display sMessage in PigPen
// ===========================================
// sMessage = message to display
// oData = target group
// fSpeed = second per letter
...
// nVFX = VFX_BEAM_* constant
// fRotate = angle of rotation
void TextPigPenMessage(string sMessage, object oData, float fSpeed=2.0f, float fLifetime=0.0f, float fFontHeight=0.5f, float fFontWidth=0.25f, int nVFX=VFX_BEAM_FIRE_W_SILENT, float fRotate=0.0f);
...
/*
   ...
   ==================================
*/
void gao_CreateTextGrid(object oNode, float fFontHeight, float fFontWidth, object oData, float fLifetime, float fRotate)
{
   object oArea = GetArea(oNode);
   vector vPos = GetPosition(oNode);
   fRotate += GetFacing(oNode);
   vector vFacing = AngleToVector(fRotate + 90.0);
   vector vLedNode;
   object oLedNode;
   fFontWidth /= 2.0;
   fFontHeight /= 2.0;
   ...
   for (i=0; i<3; i++)
   {
      ...
      for (j=0; j<3; j++)
      {
         ...
         oLedNode = CreateObject(OBJECT_TYPE_PLACEABLE, "invisobj", Location(oArea, vLedNode, fRotate), FALSE, "PSC_X_TEXTMESSAGE");
...
void gao_BeamPigPenLetter(object oNode, string sAlphabet, int nVFX)
{
   ...
   object oNode5 = GetLocalObject(oNode, "led11");    // 7 8 9
   object oNode6 = GetLocalObject(oNode, "led21");    // 4 5 6
   object oNode7 = GetLocalObject(oNode, "led02");    // 1 2 3
   ...
   {
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode6, BODY_NODE_CHEST), oNode1);
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode6, BODY_NODE_CHEST), oNode7);
   }
   else if (sAlphabet == "u")
   {
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode4, BODY_NODE_CHEST), oNode9);
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode4, BODY_NODE_CHEST), oNode3);
   }
   else if (sAlphabet == "v")
   {
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode1, BODY_NODE_CHEST), oNode8);
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode3, BODY_NODE_CHEST), oNode8);
   }
   else if (sAlphabet == "w")
   {
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode2, BODY_NODE_CHEST), oNode7);
   ...
   {
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode6, BODY_NODE_CHEST), oNode7);
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode6, BODY_NODE_CHEST), oNode4);
      ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectBeam(nVFX, oNode6, BODY_NODE_CHEST), oNode1);
   }
   else if (sAlphabet == "y")
   {
...
void gao_PigPenAlphabetPermanent(object oNode, string sAlphabet, int nVFX, float fLifetime)
{
   gao_BeamPigPenLetter(oNode, sAlphabet, nVFX);
   if (fLifetime > 0.0) DelayCommand(fLifetime, gao_DestroyTextGrid(oNode));
}

void TextMessage(string sMessage, object oData=OBJECT_SELF, float fSpeed=2.0f, float fLifetime=0.0f, float fFontHeight=0.5f, float fFontWidth=0.25f, int nVFX=VFX_BEAM_FIRE_W_SILENT, float fRotate=0.0f)
{
   int i;
   object oNode;
   ...
   for (i=0; i<nLength; i++)
   {
      ...
      {
         gao_CreateTextGrid(oNode, fFontHeight, fFontWidth, oData, fLifetime, fRotate);
...
void TextPigPenMessage(string sMessage, object oData=OBJECT_SELF, float fSpeed=2.0f, float fLifetime=0.0f, float fFontHeight=0.5f, float fFontWidth=0.25f, int nVFX=VFX_BEAM_FIRE_W_SILENT, float fRotate=0.0f)
{
   int i;
   object oNode;
   ...
   for (i=0; i<nLength; i++)
   {
      ...
      {
         gao_CreateTextGrid(oNode, fFontHeight, fFontWidth, oData, fLifetime, fRotate);
         DelayCommand(fSpeed*IntToFloat(i) + 1.0, gao_PigPenAlphabetPermanent(oNode, GetSubString(sMessage, i, 1), nVFX, fLifetime));
      }
      else break;
   }
}
