...
int chooseTactics(object oIntruder)
{
    ...
    if(oIntruder == OBJECT_INVALID){
        WriteTimestampedLogEntry("chooseTactics(object oIntruder): Entered with invalid target");
    }
    ...
    switch (nClass)
    {
        ...
        case CLASS_TYPE_BARBARIAN:
        {
           // SpawnScriptDebugger();
            ...
            if (GetLocalInt(OBJECT_SELF, "PL_AI_RAGE")
                && GetHasFeatEffect(FEAT_BARBARIAN_RAGE) == FALSE){
            ...
            nOffense = 100; nCompassion = -100; nMagic = -100; break;
    ...
    if(GetHasFeat(FEAT_EPIC_SPELL_HELLBALL)){
        SpeakString("Epic Spell: Hellball");
        ClearActions(2000);
        ActionUseFeat(FEAT_EPIC_SPELL_HELLBALL, GetNearestEnemy());
   ...
   // **************************************
    ...
    if (InvisibleTrue(OBJECT_SELF) == TRUE && nMagic > 0)
    {
        ...
        // PL: Modified to have max hit points.
    ...
    // **************
    // PL: Monsters never go invisible.
    ...
    if(GetLocalInt(OBJECT_SELF, "PL_AI_BEHOLDER")){
        if(GetDistanceToObject(oIntruder) > 5.0){ // Try to use rays.
    ...
    // Only do dragon breath once.
    if (GetLocalInt(OBJECT_SELF, "NW_GENERIC_DRAGONS_BREATH") == 0
        && TalentDragonCombat(oIntruder)) {
        return 99;
    }
...
void DetermineCombatRound(object oIntruder = OBJECT_INVALID, int nAI_Difficulty = 10)
{
     //SpeakString("************** DETERMINE COMBAT ROUND START *************");
