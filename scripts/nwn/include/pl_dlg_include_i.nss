...
//
// PL: Added delay for pausing and resuming a conversation to allow
// time for complex initializations to load.
void SetDlgPageString( string page, float fDelay = 0.2f );
...
void SetDlgPageString( string page, float fDelay = 0.2f)
{
    ...
    AssignCommand(oSpeaker, ActionPauseConversation());
    DelayCommand(fDelay, AssignCommand(oSpeaker, ActionResumeConversation()));
               ...
               int bPrivate = FALSE, int bPlayHello = TRUE, int bZoom = TRUE )
{
    ...
    string dlg = "pl_dlg_converse";
    if( !bZoom )
        dlg = "pl_dlg_converse_nz";
