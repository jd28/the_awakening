...
int VerifyPlayernameAgainstCDKey(object oPlayer) {
  ...
  string addkey = GET("cdkey:add:"+sUnencoded);
  if(GetStringLength(addkey) > 0) {
    DEL("cdkey:add:"+sUnencoded);
    ...
    if (FindSubString(sStoredKey, sKey) == -1) {
      ...
      if (nKeyLength > 61) {
        ...
        sSQL = "UPDATE nwn.players SET cdkey='" + sStoredKey + "-" + sKey + "' WHERE account='" + sPlayer + "'";
        SQLExecDirect(sSQL);
        DelayCommand(25.0, FloatingTextStringOnCreature("New CD Key Successfully Added!", oPlayer, FALSE));
      }
    } else {
      DelayCommand(25.0,
        FloatingTextStringOnCreature("CD Key Addition Failed! This key already listed for this account!",
                                     oPlayer, FALSE));
    }
