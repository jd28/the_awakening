...
object bkAcquireTarget()
{
    ...
    object oOtherTarget = GetLocalObject(OBJECT_SELF, "PL_AI_TARGET");
    int nTimeSinceAcquire = GetLocalInt(GetModule(), "uptime") - GetLocalInt(OBJECT_SELF, "PL_AI_TIME_TARGET_ACQUIRED");

    if (oOtherTarget != OBJECT_INVALID &&
        nTimeSinceAcquire <= 6 &&
        GetArea(OBJECT_SELF) == GetArea(oOtherTarget))
...
object ChooseNewTarget()
{
    ...
    SetLocalInt(OBJECT_SELF, "PL_AI_TIME_TARGET_ACQUIRED", GetLocalInt(GetModule(), "uptime"));
    SetLocalObject(OBJECT_SELF, "PL_AI_TARGET", oTarget);
