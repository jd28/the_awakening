...
object oStatue2 = GetObjectByTag("dno_Inv_Statue_2");
...
effect eFire = EffectBeam(VFX_BEAM_FIRE, oChain2, BODY_NODE_CHEST);

    ActionPlayAnimation(ANIMATION_PLACEABLE_ACTIVATE);

    DelayCommand(3.0, SoundObjectPlay(oSound));
DelayCommand(3.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue2));
    DelayCommand(3.5, SoundObjectPlay(oSound));
DelayCommand(3.5, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue2));
    DelayCommand(4.0, SoundObjectPlay(oSound));
DelayCommand(4.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue2));
    DelayCommand(4.5, SoundObjectPlay(oSound));
DelayCommand(4.5, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue2));

string sFlame = "dno_kad_flame";
location lFlame = lStatue2;

    DelayCommand(6.0, CreateFlame(sFlame, lFlame));

DelayCommand(7.0, ApplyEffectToObject(DURATION_TYPE_PERMANENT, eFire, oChain2a));
