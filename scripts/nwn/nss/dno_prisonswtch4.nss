...
object oStatue4 = GetObjectByTag("dno_Inv_Statue_4");
...
location lStatue4 = GetLocation(oStatue4);
location lWP= GetLocation(oWP);

effect eFlame = EffectVisualEffect(VFX_IMP_MIRV_FLAME);
effect eIce = EffectVisualEffect(VFX_FNF_MYSTICAL_EXPLOSION);
effect eFire = EffectBeam(VFX_BEAM_FIRE, oChain4, BODY_NODE_CHEST);

    ActionPlayAnimation(ANIMATION_PLACEABLE_ACTIVATE);

    DelayCommand(3.0, SoundObjectPlay(oSound));
DelayCommand(3.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue4));
    DelayCommand(3.5, SoundObjectPlay(oSound));
DelayCommand(3.5, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue4));
    DelayCommand(4.0, SoundObjectPlay(oSound));
DelayCommand(4.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue4));
    DelayCommand(4.5, SoundObjectPlay(oSound));
DelayCommand(4.5, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFlame, oStatue4));

string sFlame = "dno_kerso_flame";
location lFlame = lStatue4;

    DelayCommand(6.0, CreateFlame(sFlame, lFlame));

DelayCommand(7.0, ApplyEffectToObject(DURATION_TYPE_PERMANENT, eFire, oChain4a));
