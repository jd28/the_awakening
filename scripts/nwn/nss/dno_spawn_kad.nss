...
object oBlood = GetItemPossessedBy(OBJECT_SELF, "dno_Kad_Blood");
        if (!GetIsObjectValid(oBlood)) return;

object oBook =  GetItemPossessedBy(OBJECT_SELF, "dno_Kad_Book");
        if (!GetIsObjectValid(oBook)) return;

object oEye =  GetItemPossessedBy(OBJECT_SELF, "dno_Kad_Eye");
...
oTarget = GetWaypointByTag("dno_WP_Kadmilos");
...
string sCreature = "dno_kadmilos_1";
