...
object oBlood = GetItemPossessedBy(OBJECT_SELF, "dno_Kersa_Blood");
        if (!GetIsObjectValid(oBlood)) return;

object oBook =  GetItemPossessedBy(OBJECT_SELF, "dno_Kersa_Book");
        if (!GetIsObjectValid(oBook)) return;

object oEye =  GetItemPossessedBy(OBJECT_SELF, "dno_Kersa_Eye");
...
oTarget = GetWaypointByTag("dno_WP_Kersa");
