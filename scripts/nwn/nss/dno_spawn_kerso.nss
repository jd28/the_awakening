...
object oBlood = GetItemPossessedBy(OBJECT_SELF, "dno_Kerso_Blood");
        if (!GetIsObjectValid(oBlood)) return;

object oBook =  GetItemPossessedBy(OBJECT_SELF, "dno_Kerso_Book");
        if (!GetIsObjectValid(oBook)) return;

object oEye =  GetItemPossessedBy(OBJECT_SELF, "dno_Kerso_Eye");
...
oTarget = GetWaypointByTag("dno_WP_Kersos");
