...
void gao_ActionCreateLocalObject(string sTemplate, location lLocation, string sNumber, object oData, int nDurationType, int nVFX, float fDuration, float fWait, float fLifetime)
{
    ...
    if(!GetIsObjectValid(oData))
        SendMessageToPC(GetFirstPC(), "gao_ActionCreateLocalObject: Invalid oData");

    if(!GetIsObjectValid(oObject))
        SendMessageToPC(GetFirstPC(), "gao_ActionCreateLocalObject: Invalid, Resref: "+sTemplate);

    SendMessageToPC(GetFirstPC(), "sNumber: " + sNumber);
    */
    SetLocalObject(oData, sNumber, oObject);
...
void gao_ActionApplyLocalBeamEffect(object oData, string sNumber1, string sNumber2, int nDurationType, int nVFX, float fDuration)
{
    ...
    if(oNode1 == OBJECT_INVALID)
        SendMessageToPC(GetFirstPC(), "Invalid");

    if(oNode2 == OBJECT_INVALID)
        SendMessageToPC(GetFirstPC(), "Invalid");

    //SendMessageToPC(GetFirstPC(), GetTag(oNode1)+" "+GetTag(oNode2)+" "+IntToString(nDurationType)+" "+IntToString(nVFX));
