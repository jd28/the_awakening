...
void main(){
    ...
    else if (oTarget == GetAreaOfEffectCreator())
    {
        ...
        if ( ResistSpell(OBJECT_SELF,oTarget) != 2 )
        {
            if(GetSubRace(oTarget) == "Gnome-svirfneblin"){
                int nSR = GetSkillRank(SKILL_CONCENTRATION, oTarget, TRUE) * 2;
                if(nSR > 80) nSR = 80;
                int nConceal = 10 + GetSkillRank(SKILL_HIDE, oTarget, TRUE);
                if(nConceal > 60) nConceal = 60;

                eLink2 = EffectLinkEffects(eLink2, EffectSpellResistanceIncrease(nSR));
                eLink2 = EffectLinkEffects(eLink2, EffectConcealment(nConceal));
            }
