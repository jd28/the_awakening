...
void Drown(object oTarget)
{
        ...
        &&(GetRacialType(oTarget) != RACIAL_TYPE_ELEMENTAL))
    {
        ...
        if(!GetIsPC(OBJECT_SELF)){
            ...
            ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDamage(nDamage), oTarget);
        }
        else if(MySavingThrow(SAVING_THROW_FORT, oTarget, 20) == FALSE)
        {
            //Apply the VFX impact and damage effect
            ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
            //Set damage effect to kill the target
...
void main ()
{
    int nDamage = GetCurrentHitPoints() / 4;
