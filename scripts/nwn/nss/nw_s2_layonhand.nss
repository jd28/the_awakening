...
void main(){
    ...
    if(nLevel >= 30){
        nHeal = GetMaxHitPoints(si.target)-GetCurrentHitPoints(si.target);
    ...
    //--------------------------------------------------------------------------
    if(GetRacialType(si.target) == RACIAL_TYPE_UNDEAD || GetLevelByClass(CLASS_TYPE_UNDEAD,si.target)>0)
    {
        //Fire cast spell at event for the specified target
        SignalEvent(si.target, EventSpellCastAt(OBJECT_SELF, SPELLABILITY_LAY_ON_HANDS));
        //Make a ranged touch attack
        nTouch = TouchAttackMelee(si.target,TRUE);
        ...
        //----------------------------------------------------------------------
        int nResist = MyResistSpell(OBJECT_SELF,si.target);
        if (nResist == 0 )
        {
            if(nTouch > 0)
            {
                ...
                ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis2, si.target);
            }
        }
    }
    else
    {
        ...
        SignalEvent(si.target, EventSpellCastAt(OBJECT_SELF, SPELLABILITY_LAY_ON_HANDS, FALSE));
