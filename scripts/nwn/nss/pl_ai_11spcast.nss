...
void KillTarget(object oTarget);
void KillTarget(object oTarget){
...
void main(){
    ...
    if(GetLastSpell() == SPELL_LIGHT &&
       GetTag(OBJECT_SELF) == "pl_living_darkness")
    {
        ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_DIVINE_STRIKE_HOLY), OBJECT_SELF);
        int nHP = GetCurrentHitPoints();
        int nDamage = d20(60);
        if(nHP > nDamage){
            SetCurrentHitPoints(OBJECT_SELF, nHP - nDamage);
        }
        else{
            AssignCommand(oCaster, KillTarget(oSelf));
        }
        return;
    }
    ...
    if(GetLastSpellHarmful())
    {
        ...
        if (GetFactionEqual(oCaster, OBJECT_SELF) == TRUE)
        {
            ...
            DelayCommand(1.2, ActionDoCommand(PL_AIDetermineCombatRound(OBJECT_INVALID)));
        ...
        if (MatchAreaOfEffectSpell(GetLastSpell()) == TRUE)
        {
            ...
            switch (nAI)
            {
                ...
                case X2_SPELL_AOEBEHAVIOR_FLEE:
                         ...
                         DelayCommand(1.2, ActionDoCommand(PL_AIDetermineCombatRound()));
                         bAttack = FALSE;
                         break;
    ...
    else
    {
        ...
        // PL: Changed to there is a Percieced Enemy
        if (GetNearestPerceivedEnemy() != OBJECT_INVALID){
            PL_AIDetermineCombatRound();
        }
    }
