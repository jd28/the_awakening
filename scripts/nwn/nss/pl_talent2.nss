...
int TryTalent(talent tUse, object oTarget=OBJECT_SELF, object oCaster=OBJECT_SELF)
{
    ...
    WriteTimestampedLogEntry("TryTalent Successful Exit");
    return TRUE;
...
int TalentUseProtectionOnSelf()
{
    WriteTimestampedLogEntry("TalentUseProtectionOnSelf Enter");
    talent tUse;
    int nType, nIndex;
    int bValid = FALSE;
    int nCR = GetCRMax();
    ...
    if(!GetIsTalentValid(tUse)) {
        tUse = GetCreatureTalent(TALENT_CATEGORY_BENEFICIAL_PROTECTION_SINGLE,
                                     GetCRMax());
        if(GetIsTalentValid(tUse))  {
            //WriteTimestampedLogEntry("I have found a way to protect my self");
            bValid = TRUE;
        }
    } else {
        //WriteTimestampedLogEntry("I have found a way to protect my self");
    ...
    if (bValid == TRUE) {
        if (TryTalent(tUse)) {
            WriteTimestampedLogEntry("TalentUseProtectionOnSelf Successful Exit");
            return TRUE;
        }
    }

    WriteTimestampedLogEntry("TalentUseProtectionOnSelf Failed Exit");
    return FALSE;
}
...
int TalentUseProtectionOthers(object oDefault=OBJECT_INVALID)
{
    WriteTimestampedLogEntry("TalentUseProtectionOthers Enter");
    talent tUse, tMass;
    ...
    while(GetIsObjectValid(oTarget)) {
        if(GetIsTalentValid(tMass) && CheckFriendlyFireOnTarget(oTarget) > 2) {
            if (TryTalent(tMass, oTarget)) {
                WriteTimestampedLogEntry("TalentUseProtectionOthers Successful Exit");
                return TRUE;
            }
        }
    ...
    WriteTimestampedLogEntry("TalentUseProtectionOthers Failed Exit");
    return FALSE;
}
...
int TalentEnhanceOthers()
{
    WriteTimestampedLogEntry("TalentEnhanceOthers Enter");
    talent tUse, tMass;
    int nType, nFriends, nIndex;
    int nCnt = 1;
    int bValid = FALSE;
    ...
    while(GetIsObjectValid(oTarget)) {
        if(GetIsTalentValid(tMass)) {
            if(CheckFriendlyFireOnTarget(oTarget) > 2) {
                if (TryTalent(tMass, oTarget)) {
                    WriteTimestampedLogEntry("TalentEnhanceOthers Successful Exit");
                    return TRUE;
                }
            }
        }
    ...
    WriteTimestampedLogEntry("TalentEnhanceOthers Failed Exit");
    return FALSE;
}
...
int TalentUseEnhancementOnSelf()
{
    WriteTimestampedLogEntry("TalentUseEnhancementOnSelf Enter");
    talent tUse;
    int nType;
    int bValid = FALSE;
    int nIndex;
    ...
    if(bValid == TRUE) {
        // pl: Never use invisibility...
        if (GetIdFromTalent(tUse) == SPELL_INVISIBILITY) {
            WriteTimestampedLogEntry("Decided not to use Invisibility");
            return FALSE; // BRENT JULY 2002: There is a 50% chance that
                          // they will not use invisibility if they have it
        }

        if (TryTalent(tUse)) {
            WriteTimestampedLogEntry("TalentUseEnhancementOnSelf Successful Exit");
            return TRUE;
        }
    }

    WriteTimestampedLogEntry("TalentUseEnhancementOnSelf Failed Exit");
    return FALSE;
}
...
int genericDoHarmfulRangedAttack(talent tUse, object oTarget)
{
    WriteTimestampedLogEntry("BK: genericDoharmfulRangedAttack");
    ...
    if (GetTypeFromTalent(tUse) == TALENT_TYPE_SPELL)
    {
        ...
        {
            WriteTimestampedLogEntry("BK: inside of dispel magic");
            effect eEffect = GetFirstEffect(oTarget);
            while (GetIsEffectValid(eEffect) == TRUE)
            {
                WriteTimestampedLogEntry("BK: Valid effect");
                ...
                if (nEffectID == -1)
                {
                 WriteTimestampedLogEntry("BK: conditions NOT met");
                }
                else
                {
                    WriteTimestampedLogEntry("BK: conditions met");
                 bConditionsMet = TRUE;
                }
    ...
    {
        WriteTimestampedLogEntry("BK: bCOnditionsMet == TRUE");
        //DebugPrintTalentID(tUse);
        WriteTimestampedLogEntry("TalentMeleeAttacked Successful Exit");
...
int genericAttemptHarmfulRanged(talent tUse, object oTarget)
{
    ...
    {
        WriteTimestampedLogEntry("2164: Can try to do a DoHarmfulRangedAttack");
    ...
    // else
    WriteTimestampedLogEntry("BK: Harmful Ranged will return false");
    return FALSE;
}

int TalentMeleeAttacked(object oIntruder = OBJECT_INVALID)
{
    WriteTimestampedLogEntry("TalentMeleeAttacked Enter");
    talent tUse;
    ...
    */
    ...
    if(nMelee == 1)
    {
        ...
        if(GetIsTalentValid(tUse))
        {
                ...
                GetTypeFromTalent(tUse) != TALENT_TYPE_SPELL )
            {
                ...
                //DebugPrintTalentID(tUse);
                WriteTimestampedLogEntry("TalentMeleeAttacked HARMFUL TOUCH Successful Exit");
                bkTalentFilter(tUse, oTarget);
                return TRUE;
            }
        }
        ...
        if(GetIsTalentValid(tUse))
        {
                ...
                GetTypeFromTalent(tUse) != TALENT_TYPE_SPELL )
            {
                ...
                //DebugPrintTalentID(tUse);
                WriteTimestampedLogEntry("TalentMeleeAttacked AREAEFFECT D Successful Exit");
                bkTalentFilter(tUse, oTarget);
                return TRUE;
            }
        }
    ...
    else if (nMelee > 1)
    {
        ...
        if(GetIsTalentValid(tUse))
        {
                ...
                GetTypeFromTalent(tUse) != TALENT_TYPE_SPELL )
             {
                ...
                //DebugPrintTalentID(tUse);
                WriteTimestampedLogEntry("TalentMeleeAttacked AREA EFFECT DSuccessful Exit");
                bkTalentFilter(tUse, oTarget);
                return TRUE;
            }
        }
        ...
        if(GetIsTalentValid(tUse))
        {
            ...
            if (genericAttemptHarmfulRanged(tUse, oTarget) == TRUE)
            {
                WriteTimestampedLogEntry("TalentMeleeAttacked HARMFUL RANGED Successful Exit");
    ...
    WriteTimestampedLogEntry("TalentMeleeAttacked Failed Exit");
    return FALSE;
}
...
int TalentRangedAttackers(object oIntruder = OBJECT_INVALID)
{
    WriteTimestampedLogEntry("TalentRangedAttackers Enter");
    ...
    if(GetIsObjectValid(oTarget) && GetDistanceBetween(oTarget, OBJECT_SELF) > 5.0)
    {
        if(CheckFriendlyFireOnTarget(oTarget) > 0)
        {
            ...
            if(GetIsTalentValid(tUse))
            {
                    ...
                    GetTypeFromTalent(tUse) != TALENT_TYPE_SPELL )
                {
                    ...
                    // DebugPrintTalentID(tUse);
                    WriteTimestampedLogEntry("TalentRangedAttackers Successful Exit");
                    bkTalentFilter(tUse, oTarget);
                    return TRUE;
                }
            }
        ...
        else if(CheckEnemyGroupingOnTarget(oTarget) > 0)
        {
            ...
            WriteTimestampedLogEntry("2346 : Choose Talent here " + IntToString(nCR));
                if( (GetTypeFromTalent(tUse) == TALENT_TYPE_SPELL &&
                    !GetHasSpellEffect(GetIdFromTalent(tUse), oTarget) &&
                    !CompareLastSpellCast(GetIdFromTalent(tUse)) ) ||
                    GetTypeFromTalent(tUse) != TALENT_TYPE_SPELL )
    ...
    WriteTimestampedLogEntry("TalentRangedAttackers Failed Exit");
    return FALSE;
}
...
int TalentRangedEnemies(object oIntruder = OBJECT_INVALID)
{
    WriteTimestampedLogEntry("TalentRangedEnemies Enter");
    talent tUse;
    object oTarget = oIntruder;
    int nCR = GetCRMax();
    if(!GetIsObjectValid(oIntruder))
    ...
    if(GetIsObjectValid(oTarget))
    {
        ...
        if(CheckFriendlyFireOnTarget(oTarget) > 0 &&  nEnemy > 0)
        {
            ...
            if(GetIsTalentValid(tUse))
            {
                    ...
                    GetTypeFromTalent(tUse) != TALENT_TYPE_SPELL )
                {
                    ...
                    // DebugPrintTalentID(tUse);
                    WriteTimestampedLogEntry("TalentRangedEnemies Successful Exit");
        ...
        else if(nEnemy > 0)
        {
            ...
            if(GetIsTalentValid(tUse))
            {
                WriteTimestampedLogEntry("2428 : Choose Talent here " + IntToString(nCR));
    ...
    WriteTimestampedLogEntry("TalentRangedEnemies Failed Exit");
    return FALSE;
}
// * Wrapper function so that I could add a variable to allow randomization
// * to the AI.
...
int TalentSpellAttack(object oIntruder)
{
    WriteTimestampedLogEntry("Talent Spell Attack Enter");
    talent tUse;
    object oTarget = oIntruder;
    if(!GetIsObjectValid(oTarget) || GetArea(oTarget) != GetArea(OBJECT_SELF) || GetPlotFlag(OBJECT_SELF) == TRUE)
    {
        oTarget = GetLastHostileActor();
        WriteTimestampedLogEntry("Last Hostile Attacker: " + ObjectToString(oTarget));
        if(!GetIsObjectValid(oTarget)
            || GetIsDead(oTarget)
        ...
        {
            oTarget = GetNearestSeenEnemy();
            WriteTimestampedLogEntry("Get Nearest Seen or Heard: " + ObjectToString(oTarget));
            if(!GetIsObjectValid(oTarget))
            {
                return FALSE;
            }
    ...
    {
        WriteTimestampedLogEntry("Discriminant AOE not valid");
    ...
    if(GetIsTalentValid(tUse))
    {
             ...
             && !GetHasSpellEffect(GetIdFromTalent(tUse), oTarget)) )
        {
            ...
            WriteTimestampedLogEntry("Spell Attack Discriminate Chosen");
            bValid = TRUE;
        }
    }
    ...
    if(bValid == FALSE)
    {
        ...
        {     //  SpawnScriptDebugger();
            ...
            if (genericAttemptHarmfulRanged(tUse, oTarget) == TRUE)
            {
                WriteTimestampedLogEntry("Spell Attack Harmful Ranged Chosen");
    ...
    if (bValid == TRUE)
    {
        ...
        // DebugPrintTalentID(tUse);
        WriteTimestampedLogEntry("Talent Spell Attack Successful Exit");
    ...
    WriteTimestampedLogEntry("Talent Spell Attack Failed Exit");
    /* JULY 2003
...
int TalentSummonAllies()
{
    WriteTimestampedLogEntry("TalentSummonAllies Enter");
    ...
    if(!GetIsObjectValid(GetAssociate(ASSOCIATE_TYPE_SUMMONED)))
    {
        ...
        if(GetIsTalentValid(tUse))
        {
            ...
            {
                WriteTimestampedLogEntry("TalentSummonAllies Successful Exit");
        ...
        //   else
            WriteTimestampedLogEntry("No valid Talent for summoning Allies with Difficulty " + IntToString(GetLocalInt(OBJECT_SELF,"NW_L_COMBATDIFF")));
    }
    WriteTimestampedLogEntry("TalentSummonAllies Failed Exit");
    return FALSE;
}
...
//       GetRacialType(OBJECT_SELF) != RACIAL_TYPE_CONSTRUCT ||
       ...
       GetRacialType(OBJECT_SELF) != RACIAL_TYPE_ANIMAL)
    {
        ...
        WriteTimestampedLogEntry("TalentHealingSelf Enter");
        ...
        if( (nCurrent < nBase) || (bForce == TRUE) )
        {
            ...
            if(GetIsTalentValid(tUse))
            {
                WriteTimestampedLogEntry("Talent\ Successful Exit");
                bkTalentFilter(tUse, OBJECT_SELF);
                return TRUE;
            }
            else
            {
                ...
                if(GetIsTalentValid(tUse))
                {
                    WriteTimestampedLogEntry("TalentHealingSelf Successful Exit");
                    bkTalentFilter(tUse, OBJECT_SELF);
                    return TRUE;
                }
            }
    ...
    WriteTimestampedLogEntry("TalentHealingSelf Failed Exit");
    return FALSE;
}
...
int TalentHeal(int nForce = FALSE, object oTarget = OBJECT_SELF)
{
    WriteTimestampedLogEntry("TalentHeal Enter");
...
/*    if (nForce == TRUE)
    ...
    if(GetAssociateHealMaster() || nForce == TRUE)
    {
        ...
        {
            WriteTimestampedLogEntry("TalentHeal Successful Exit");
            return TRUE;
        }
    }
    */
    ...
    {
        bkTalentFilter(tUse, oTarget);
        WriteTimestampedLogEntry("TalentHeal (MASTER) Successful Exit");
        return TRUE;
    }
    // * Heal self
    if(nCurrent < nBase)
    {
        ...
        {
            WriteTimestampedLogEntry("TalentHeal Failed Exit");
            return FALSE;
        }
...
int TalentMeleeAttack(object oIntruder = OBJECT_INVALID)
{
    WriteTimestampedLogEntry("TalentMeleeAttack Enter");
    WriteTimestampedLogEntry("Intruder: " + ObjectToString(oIntruder));
        ...
        || GetAssociateState(NW_ASC_MODE_DYING, oTarget))
    {
        ...
        WriteTimestampedLogEntry("Attempted Attack Target: " + ObjectToString(oTarget));
        ...
        {
            oTarget = GetLastHostileActor();
            WriteTimestampedLogEntry("Last Attacker: " + ObjectToString(oTarget));
            if(!GetIsObjectValid(oTarget)
               || GetIsDead(oTarget)
               || GetArea(oTarget) != GetArea(OBJECT_SELF)
               || GetPlotFlag(OBJECT_SELF) == TRUE)
            {
                ...
                WriteTimestampedLogEntry("Get Nearest Perceived: "                              + ObjectToString(oTarget));
                if(!GetIsObjectValid(oTarget)) {
                    WriteTimestampedLogEntry("TALENT MELEE ATTACK FAILURE EXIT"                                  + " - NO TARGET FOUND");
                    return FALSE;
                }
            }
        }
    ...
    WriteTimestampedLogEntry("Selected Target: " + ObjectToString(oTarget));
    ...
    WriteTimestampedLogEntry("nDiff = " + IntToString(nDiff));
...
*/
    ...
    if(nDiff < 10)
    {
        ...
        tUse = GetCreatureTalent(TALENT_CATEGORY_HARMFUL_MELEE,
                                     GetCRMax());
        WriteTimestampedLogEntry("Melee Talent Valid = "+ IntToString(GetIsTalentValid(tUse)));
        WriteTimestampedLogEntry("Feat ID = " + IntToString(GetIdFromTalent(tUse)));

        if(GetIsTalentValid(tUse)
           && VerifyDisarm(tUse, oTarget)
           && VerifyCombatMeleeTalent(tUse, oTarget))
        {
            WriteTimestampedLogEntry("TalentMeleeAttack: Talent Use Successful");
        ...
        {
            WriteTimestampedLogEntry("TalentMeleeAttack Successful Exit");
            WrapperActionAttack(oTarget);
            return TRUE;
        }
    }
    ...
    WriteTimestampedLogEntry("TALENT MELEE ATTACK FAILURE EXIT - THIS IS VERY BAD");
    return FALSE;
}
...
int TalentSneakAttack()
{
     WriteTimestampedLogEntry("TalentSneakAttack Enter");
     ...
     if(GetHasFeat(FEAT_SNEAK_ATTACK))
     {
         ...
         if (GetIsObjectValid(oFriend)) {
             ...
             if(GetIsObjectValid(oTarget)
                && !GetIsDead(oTarget)
                && !GetAssociateState(NW_ASC_MODE_DYING, oTarget)) {
                 WriteTimestampedLogEntry("TalentSneakAttack Successful Exit");
                 TalentMeleeAttack(oTarget);
                 return TRUE;
             }
         }
     }
     WriteTimestampedLogEntry("TalentSneakAttack Failed Exit");
     return FALSE;
}
...
int TalentFlee(object oIntruder = OBJECT_INVALID)
{
    WriteTimestampedLogEntry("TalentFlee Enter");
    object oTarget = oIntruder;
    if(!GetIsObjectValid(oIntruder))
    {
        oTarget = GetLastHostileActor();
        if(!GetIsObjectValid(oTarget) || GetIsDead(oTarget))
        {
            ...
            if(!GetIsObjectValid(oTarget))
            {
                WriteTimestampedLogEntry("TalentFlee Failed Exit");
                return FALSE;
            }
        }
    }
    WriteTimestampedLogEntry("TalentFlee Successful Exit");
...
int TalentUseTurning()
{
    WriteTimestampedLogEntry("TalentUseTurning Enter");
    int nCount;
    if(GetHasFeat(FEAT_TURN_UNDEAD))
    {
        ...
        if(nCount > 0)
        {
            WriteTimestampedLogEntry("TalentUseTurning Successful Exit");
    ...
    WriteTimestampedLogEntry("TalentUseTurning Failed Exit");
    return FALSE;
}
...
int TalentPersistentAbilities()
{
    WriteTimestampedLogEntry("TalentPersistentAbilities Enter");
    ...
    if(GetIsTalentValid(tUse))
    {
        ...
        {
            WriteTimestampedLogEntry("TalentPersistentAbilities Successful Exit");
            bkTalentFilter(tUse, OBJECT_SELF);
            return TRUE;
        }
    }
    WriteTimestampedLogEntry("TalentPersistentAbilities Failed Exit");
    return FALSE;
}
...
int TalentAdvancedBuff(float fDistance, int bInstant = TRUE)
{
    WriteTimestampedLogEntry("TalentAdvancedBuff Enter");
    ...
    //Shields
    if (GetHasSpell(SPELL_MESTILS_ACID_SHEATH)&& !GetHasSpellEffect(SPELL_MESTILS_ACID_SHEATH))
    ...
    if (GetHasSpell(SPELL_WOUNDING_WHISPERS)&& !GetHasSpellEffect(SPELL_WOUNDING_WHISPERS))
        ActionCastSpellAtObject(SPELL_DEATH_ARMOR, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, bInstant);
    ...
    if(GetHasSpell(SPELL_DIVINE_FAVOR)&& !GetHasSpellEffect(SPELL_DIVINE_FAVOR))
        ActionCastSpellAtObject(SPELL_DIVINE_FAVOR, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, bInstant);
    ...
    for(iCnt = SPELLABILITY_AURA_BLINDING; iCnt <= SPELLABILITY_AURA_UNNATURAL; iCnt++){
        if(GetHasSpell(iCnt) && !GetHasSpellEffect(iCnt)){
            ActionCastSpellAtObject(iCnt, OBJECT_SELF, METAMAGIC_NONE, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
        }
    }
    if(GetHasSpell(SPELLABILITY_DRAGON_FEAR) && !GetHasSpellEffect(SPELLABILITY_DRAGON_FEAR))
        ActionCastSpellAtObject(SPELLABILITY_DRAGON_FEAR, OBJECT_SELF, METAMAGIC_NONE, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
    if(GetHasSpell(SPELLABILITY_TYRANT_FOG_MIST) && !GetHasSpellEffect(SPELLABILITY_TYRANT_FOG_MIST))
        ActionCastSpellAtObject(SPELLABILITY_TYRANT_FOG_MIST, OBJECT_SELF, METAMAGIC_NONE, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
    // Aura of Hellfire
    if(GetHasSpell(761) && !GetHasSpellEffect(761))
        ActionCastSpellAtObject(761, OBJECT_SELF, METAMAGIC_NONE, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
...
int TalentBuffSelf()
{
    WriteTimestampedLogEntry("TalentBuffSelf Enter");
    //int bValid = FALSE;
    int nCRMax = GetCRMax();
    ...
    if(nType == TALENT_TYPE_SPELL && !GetHasSpellEffect(nIndex)) {
        WriteTimestampedLogEntry("TalentBuffSelf Successful Exit");
        bkTalentFilter(tUse, OBJECT_SELF);
        return TRUE;
    }

    WriteTimestampedLogEntry("TalentBuffSelf Failed Exit");
    return FALSE;
}
...
int TalentSeeInvisible()
{
    WriteTimestampedLogEntry("TalentSeeInvisible Enter");
    int nSpell;
    int bValid = FALSE;
    ...
    if(bValid == TRUE)
    {
        WriteTimestampedLogEntry("TalentSeeInvisible Successful Exit");
        ClearActions(CLEAR_X0_I0_TALENT_SeeInvisible);
        ActionCastSpellAtObject(nSpell, OBJECT_SELF);
    }
    WriteTimestampedLogEntry("TalentSeeInvisible Failed Exit");
    return bValid;
}
...
int TalentCureCondition()
{
    WriteTimestampedLogEntry("TalentCureCondition Enter");
    int nSum;
    int nCond;
    int nSpell;
    effect eEffect;
    ...
    while(GetIsObjectValid(oTarget))
    {
        if(GetIsFriend(oTarget)) {
            ...
            if (nSum != 0) {
                ...
                if(nSpell != 0) {
                    WriteTimestampedLogEntry("TalentCureCondition Successful Exit");
                    ClearActions(CLEAR_X0_I0_TALENT_AdvancedBuff);
                    ActionCastSpellAtObject(nSpell, oTarget);
                    return TRUE;
                }
    ...
    WriteTimestampedLogEntry("TalentCureCondition Failed Exit");
    return FALSE;
}
...
int TalentDragonCombat(object oIntruder = OBJECT_INVALID)
{
    WriteTimestampedLogEntry("TalentDragonCombat Enter");
    object oTarget = oIntruder;
    if(!GetIsObjectValid(oTarget))
    {
        oTarget = GetAttemptedAttackTarget();
    ...
    {
        WriteTimestampedLogEntry("TalentDragonCombat Successful Exit");
        bkTalentFilter(tUse, oTarget);
        nCnt = 0;
    ...
    WriteTimestampedLogEntry("TalentDragonCombat Failed Exit");
    //SetLocalInt(OBJECT_SELF, "NW_GENERIC_DRAGONS_BREATH", nCnt);
    return FALSE;
}
...
int TalentBardSong(){
    ...
    if(GetHasFeat(FEAT_BARD_SONGS) && !GetHasEffect(EFFECT_TYPE_SILENCE, OBJECT_SELF)){
        ...
        if(!GetHasSpellEffect(411)
           ...
           && !GetHasEffect(EFFECT_TYPE_DEAF, OBJECT_SELF))
        {
        ...
        else if(GetHasFeat(FEAT_CURSE_SONG))
        {
            ...
            if(GetIsObjectValid(oSonged) && GetDistanceToObject(oSonged) <= 6.0f
               && !GetHasEffect(EFFECT_TYPE_DEAF, oSonged))
            {
...
/*
    int iMyBardFeat;
    WriteTimestampedLogEntry("TalentBardSong Enter");
    ...
    if(!GetHasSpellEffect(411))
    {
        ...
        {
            WriteTimestampedLogEntry("TalentBardSong Successful Exit");
        ...
        // Handle Curse Song. Make sure
    // 1) The target isn't cursed already
    // 2) The target is close enough
    // 3) The mob has Curse Song in the first place
    // 4) The mob has Bard Song uses remaining
    else
    {
        object oTarget = GetNearestEnemy();
        if(!GetIsObjectValid(oTarget)) return FALSE;

        if (GetLocalInt(OBJECT_SELF, "PL_AI_CURSE_SONG")
            && !GetHasSpellEffect(SPELLABILITY_EPIC_CURSE_SONG, oTarget)
            && GetDistanceBetween(OBJECT_SELF, oTarget) <= 4.0
            && GetHasFeat(FEAT_CURSE_SONG) == TRUE
            && GetHasFeat(iMyBardFeat) == TRUE )
        {
            ClearActions(CLEAR_X0_I0_TALENT_BardSong);
            ActionUseFeat(FEAT_CURSE_SONG, OBJECT_SELF);
            return TRUE;
    ...
    WriteTimestampedLogEntry("TalentBardSong Failed Exit");
    return FALSE;
*/
}
...
int TalentAdvancedProtectSelf()
{
    WriteTimestampedLogEntry("TalentAdvancedProtectSelf Enter");
    ...
    {
        WriteTimestampedLogEntry("TalentAdvancedProtectSelf Search Self");
        tUse = GetCreatureTalent(TALENT_CATEGORY_BENEFICIAL_PROTECTION_SELF,
                                     GetCRMax());
        if(GetIsTalentValid(tUse)
           && GetMatchCompatibility(tUse, sClass, NW_TALENT_PROTECT))
        {
            ...
            WriteTimestampedLogEntry(" TalentAdvancedProtectSelfSearch Single");
            tUse =
               ...
               && GetMatchCompatibility(tUse, sClass, NW_TALENT_PROTECT))
            {
                ...
                WriteTimestampedLogEntry("TalentAdvancedProtectSelf Search Area");
                tUse =
    ...
    if(bValid == TRUE)
    {
        ...
        if(nType == TALENT_TYPE_SPELL)
        {
            if(!GetHasSpellEffect(nIndex)) {
                WriteTimestampedLogEntry("TalentAdvancedProtectSelf Successful Exit");
                bkTalentFilter(tUse, OBJECT_SELF);
                return TRUE;
            }
    ...
    WriteTimestampedLogEntry("TalentAdvancedProtectSelf Failed Exit");
    return FALSE;
}
