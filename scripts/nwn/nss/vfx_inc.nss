...
// VisualFX
void ApplyRayToObject(int nEffect, object oTarget, object oSource = OBJECT_SELF, float fDur = 1.7);
void ApplyVisualToObject(int nEffect, object oPC, float fDur = 0.0);
void ApplyVisualAtLocation(int nEffect, location lLoc, float fDur = 0.0);
void ApplyPermenatVisualToObject(int nEffect, object oTarget);
void RemoveAllVisuals(object oObj);

void ApplyVisualAtLocation(int nEffect, location lLoc, float fDur = 0.0){
    ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(nEffect), lLoc);
}

void ApplyVisualToObject(int nEffect, object oPC, float fDur = 0.0){
    if(fDur > 0.0){
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(nEffect), oPC, fDur);
    }
    else{
...
void ApplyRayToObject(int nEffect, object oTarget, object oSource = OBJECT_SELF, float fDur = 1.7){
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(nEffect, oSource, BODY_NODE_HAND), oTarget, fDur);
}
...
void RemoveAllVisuals(object oObj){
    effect eLoop = GetFirstEffect(oObj);
    //Remove flag from the player
    while (GetIsEffectValid(eLoop)){
        if (GetEffectType(eLoop)==EFFECT_TYPE_VISUALEFFECT)
            RemoveEffect(oObj, eLoop);
        eLoop = GetNextEffect(oObj);
    }
}
...
{                                                                                  //1 1 20
    ...
    {
        FloatingTextStringOnCreature(C_RED+REQUIRES_NUMBER+C_END, oPC);
        return;
    }
    ...
    if (nPos == 1)//
    {
        ...
        else
        {
            ...
            {
                FloatingTextStringOnCreature(C_RED+BADEFFECT_TYPE+C_END, oPC, FALSE);
                return;
            }
        }
    }
    ...
    {
        FloatingTextStringOnCreature(C_RED+NEED_DURATION+C_END, oPC);
        return;
    }
    effect eVFX;
    location lTarget;
    ...
    else FloatingTextStringOnCreature(C_RED+BADEFFECT_NUM+C_END, oPC, FALSE);
}
...
void ListFX(object oPC, string sType) // dur, bea, eye, imp, com, fnf
{
    ...
    {
        sName = GetVFXName(nX);
        if (GetStringLeft(sName, 7) == sType) sMessage += C_PURPLE + IntToString(nX) + ": " + sName + C_END + "\n";
    }
