    ...
    while(GetIsObjectValid(oTarget))
    {
            if (spellsIsTarget(oTarget,SPELL_TARGET_STANDARDHOSTILE,oCreator))
            {
                ...
                if (!MyResistSpell(oCreator, oTarget))
                {
                    ...
                    //Change damage according to Reflex, Evasion and Improved Evasion   +5 DC Added
                    nDamage = GetReflexAdjustedDamage(nDamage, oTarget, sSI.dc + 5, SAVING_THROW_TYPE_SONIC, oCreator);
